...
class Auth
{
    ...
    public function check($name, $uid, $type = 1, $mode = 'url', $relation = 'or')
    {
        ...
        $currentRequest = '';
        if ('url' == $mode) {
            $currentRequest = unserialize(strtolower(serialize(request()->pathinfo())));
        }
    ...
    protected function getAuthList($uid, $type)
    {
        ...
        if (2 == $this->_config['AUTH_TYPE'] && session('?AUTH_LIST_' . $uid . $t)) {
        ...
        //读取用户组所有权限规则
        $rules = AuthRule::field('condition,name')
            ->where('id', 'in', $ids)
            ->where('type', $type)
            ->where('status', 1)
            ->selectOrFail();
        ...
        if (2 == $this->_config['AUTH_TYPE']) {
            //规则列表结果保存到session
            session('AUTH_LIST_' . $uid . $t, $authList);
        }
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $userInfo[$uid] = User::where('id', $uid)->findOrEmpty();
        }
